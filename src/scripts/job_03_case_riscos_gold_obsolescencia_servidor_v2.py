...
def normalize_join_key(expr):
    ...
    """
    return F.lower(F.trim(F.regexp_replace(expr, r"\s+", " ")))
...
def treat_software_instances(df: DataFrame) -> DataFrame:
    ...
    A coluna software já vem como: name + version.
    """
    return (
        df.select(
    ...
    return (
        ...
        .withColumn(
            ...
            normalize_join_key(
                F.concat_ws(
                    " ",
                    F.col("nome_software_catalogo"),
                    F.col("versao_catalogo"),
                )
            ),
        )
